...
export const REWRITER_VERSION = 1;
...
/**
 * Troca, DELIMITADO, cada URL do índice por `prefix+localPath`. O lookbehind/
 ...
 * `<link href>` (stylesheet/icon), `srcset`, `<image href>`/`xlink:href` (SVG),
 * `url(...)` e `@import`. NÃO inclui `<a href>` — navegação não é asset.
 */
export const coletarAssetRefs = (text: string): string[] => {
  ...
  for (const m of text.matchAll(/<image\b[^>]*?\shref\s*=\s*["']([^"']+)["']/gi)) add(m[1]);
