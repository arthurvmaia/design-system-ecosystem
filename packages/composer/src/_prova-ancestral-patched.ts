...
const soltarNoSeletor = (
  ...
  let mudou = false;
  let descartou = false;
  /** O sujeito do seletor tinha um id ausente: a regra fica EXATAMENTE como estava. */
  let sujeitoAusente = false;
  const saida: string[] = [];
  try {
    selectorParser((raiz) => {
      raiz.each((sel) => {
         ...
         */
        /**
         * O SUJEITO nao se solta — e o composto MAIS A DIREITA, o elemento que a
         * regra pinta. Soltar um id ausente dali nao recupera um ancestral
         * perdido: ele REAPONTA a regra para o composto anterior, que na folha
         * ja escopada e a ancora `:where([data-ds-corpo="ds_x"])` — o proxy que
         * envolve a SECAO INTEIRA.
         */
        const ultimo = compostos.reduce((acc, g, i) => (g.length > 0 ? i : acc), -1);
        const sujeito = ultimo >= 0 ? (compostos[ultimo] ?? []) : [];
        if (sujeito.some((n) => n.type === 'id' && !presentes.has(n.value))) {
          sujeitoAusente = true;
          return;
        }
  ...
  if (sujeitoAusente) return { seletor, mudou: false };
