...
servidor.listen(0, async () => {
  ...
  for (const [w, h, nome] of [
    ...
    p.on('requestfailed', (r) =>
      erros.push(`${nome} req: ${r.url().split('/').pop()} ${r.failure()?.errorText}`),
    );
    ...
    const info = await p.evaluate(() => {
      const secs = [...document.querySelectorAll('section[data-secao]')].map((s) => ({
        id: s.dataset.secaoId,
        papel: s.dataset.secao,
        h: Math.round(s.getBoundingClientRect().height),
      }));
      const imgs = [...document.images].map((i) => ({
        src: (i.currentSrc || i.src || '(vazio)').split('/').slice(-1)[0],
        ok: i.naturalWidth > 0,
      }));
      const brutas =
        document.body.innerText.match(
          /Aris|Open Design|Mina Kovac|asimov|Elena R\.|Câmara Escura|huashu|hyperframes|Capturando/gi,
        ) || [];
      return {
        secs,
        ...
        ancoras: ['trabalhos', 'servicos', 'orcamento']
