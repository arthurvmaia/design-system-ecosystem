...
const REDES = ['instagram', 'linkedin', 'youtube', 'x', 'facebook', 'tiktok'] as const;
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <Campo label="Fonte de título">
            <input
              ...
              onChange={(e) => setB({ fontDisplay: e.target.value })}
              className={INPUT}
              style={inputStyle}
            />
          </Campo>
          <Campo label="Fonte de corpo">
            <input
              ...
              onChange={(e) => setB({ fontBody: e.target.value })}
              className={INPUT}
              style={inputStyle}
            />
          </Campo>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-3">
          ...
          />
          <div className="pt-1 text-[10px] uppercase tracking-[0.2em]" style={rotulo}>
            Redes
          </div>
          {REDES.map((rede) => (
            <MiniInput
              key={rede}
              label={rede}
              value={branding.social[rede] ?? ''}
              onChange={(v) => setB({ social: { ...branding.social, [rede]: v } })}
            />
          ))}
        </div>
      </div>
